...
const DepartmentReport = () => {
  ...
  const handleReport = async () => {
    await api.post(`report/department/${departmentSelect}`).then((res) => {
      ...
      setDataChartGeneral([
        { y: res.data.hiredCandidatesRate.toFixed(2) * 100, label: "Đậu" },
        { y: res.data.rejectedCandidatesRate.toFixed(2) * 100, label: "Bị loại" },
      ]);
      setDataChartProfile([
        { y: res.data.passingProfileRate.toFixed(2) * 100, label: "Hồ sơ đậu yêu cầu" },
        { y: res.data.failProfileRate.toFixed(2) * 100, label: "Hồ sơ không đạt yêu cầu" },
      ]);
      setDataChartPassProfile([
        {
          y: res.data.hiredCandidatePassProfRate.toFixed(2) * 100,
          label: "Đậu theo yêu cầu",
        },
        {
          y: res.data.rejectedCandidatePassProfRate.toFixed(2) * 100,
        ...
        <div>
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              <TableHead>
                <TableRow>
                  <TableCell align="center">Tổng số ứng viên ứng tuyển</TableCell>
                  <TableCell align="center">Số ứng viên đạt yêu cầu</TableCell>
                  <TableCell align="center">Số ứng viên không đạt yêu cầu</TableCell>
                  <TableCell align="center">Số ứng viên được tuyển</TableCell>
                  <TableCell align="center">Số ứng viên đạt yêu cầu bị loại</TableCell>
                  <TableCell align="center">Tổng số ứng viên bị loại</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                <TableRow
                ...
                >
                  ...
                  <TableCell align="center">
                    {departmentReport.numberOfRejectedCandidates - departmentReport.numberOfFailProfile}
                  </TableCell>
                  <TableCell align="center">
